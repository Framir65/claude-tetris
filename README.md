...
- **Pieza reto: la Tuerca** — un anillo de 3×3 con un agujero cuadrado en el centro. El hueco no se puede rellenar con ninguna otra pieza, así que cada tuerca colocada inutiliza una fila para siempre; a cambio, cualquier línea que incluya bloques de tuerca puntúa el **doble**.
...
- **Modelo del tablero**: una matriz `ROWS × COLS` donde cada celda guarda `0` (vacía) o un índice de color (1–8) que identifica la pieza. El índice `8` es la Tuerca.
- **Piezas**: definidas como matrices cuadradas. Para rotar se calcula la transposición + reverso de filas (`rotateCW`). La Tuerca es un anillo 3×3 simétrico, así que rotarla no cambia su forma.
- **Agujero de la Tuerca**: la celda central de la pieza vale `0` desde su propia definición en `PIECES`, así que el hueco cuadrado sale gratis — `drawBlock` ya se salta las celdas vacías, sin lógica adicional de recorte.
- **Detección de colisiones** (`collide`): comprueba que ninguna celda de la pieza salga del tablero ni se solape con bloques ya fijados.
- **Wall kicks** (`tryRotate`): si la rotación choca, intenta desplazar la pieza ±1 y ±2 columnas antes de descartar el giro.
- **Game loop** (`loop`): basado en `requestAnimationFrame`, acumula el tiempo transcurrido y baja la pieza una fila cuando se supera `dropInterval`.
- **Limpieza de líneas** (`clearLines`): recorre el tablero de abajo hacia arriba; cada fila completa se elimina y se inserta una vacía en la cima.
- **Puntuación**: usa la tabla clásica `[0, 100, 300, 500, 800]` multiplicada por el nivel actual; el hard drop suma 2 puntos por celda recorrida y el soft drop 1 punto por fila. Si alguna de las líneas eliminadas contiene bloques de Tuerca, la puntuación de esa limpieza se multiplica ×2.
